...
ggplot(world, aes(long, lat)) + 
  ...
  scale_colour_manual(values = c("lightskyblue","brown1","black","green","yellow"))+
  labs(title = "Flood Distribution Based on \nMain Causes and Affected Range(km^2)", x = "Longitude", 
...
```
Despite the different latitude and longitude, the flood in USA and Russia still share some same characters. For example, on five days before the flood happen, the pressure around the flood area has been increase to the similar level to the the level when the flood happened.  Among the flood area, after the flood started, the pressure level kept decreased. Also, around the third day after the flood started, in both locations, the pressure had been significantly decreased and, by looking at the width between the contour lines around the flooded area, we can also notice that the gradient of the pressure is more pronounced. However, if we looked at the contour about the pressure level 5 days after the flood ended, we can notice that interestingly, the pressure level increased a little bit, especially in USA, the pressure level was even higher than that before the flood happened. 
