...
We can see a greater fluctuation in the number of M4 floods than M6 floods, and the occurrences of floods of both degrees are highly correlated. After a view of the distribution of floods in time, we will present the spatial distribution.
...
ggplot(world, aes(long, lat)) + 
  geom_polygon(aes(group=group), fill = "White", color ="Dark Blue", size = 0.05) +
  geom_point(data=df_new4, aes(XLon, YLat, color = Cause, size = AffectedRange/1000), alpha = 0.6) +
   scale_size(range = c(0,4)) +
  scale_colour_manual(values = c("blue","brown1","black","green","yellow"))+
...
```
In performing PCA on the NOAA data, the pre-processing involved re-formating the data into an N x D matrix,  such that: N = days, and  D = lon x lat coordinates. Specifically, our goal was to do PCA over the USA, during the months of June and July, 2015 (to examine some of the floods covered in the above section). 
